...
public class PacienteControllerImpl implements IPacienteController{
									  ...
									  String generoPaciente, String ocupacionPaciente, String tipoidPaciente, String emailPaciente, String estCivilPaciente, String numficha,  String usuario)
	{
		  try { Paciente pacienteExiste = pacientesDao.getPacienteByID(tipoidPaciente, identificacionPaciente);
			  if (!(pacienteExiste == null) && (!(identificacionPaciente.equalsIgnoreCase("")))) {
				  return new JsonResponse<>( false, null, "Ya existe un paciente con ese documento de identificaci�n");
			  } else {
				  	Long id = null;
  ...
  //  Boolean darBajaPaciente(Long codigo, String usuario) {
   // }
	...
	@Override
	public JsonResponse<Paciente> getPacienteByCodigo(Long codPaciente) {
		Paciente paciente = pacientesDao.getPaciente(codPaciente);
		return new JsonResponse<>(paciente != null, paciente);
	}
